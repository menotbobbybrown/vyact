...
it('repeated retry and stop preserve one original question in UI and saved history', () => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        const retryTime = new Date(Date.UTC(2026, 8, 20, 2, attempt));
        ...
        expect(retry.userMessage).toEqual({...question, timestamp: retryTime.toISOString()});
        expect(question.timestamp).toBe('original-time');
        ...
        expect(sentHistory[sentHistory.length - 1].timestamp).toBe(retryTime.toISOString());
