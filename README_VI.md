...
Vyact tích hợp Python và chuẩn bị môi trường chạy mô hình với phiên bản được ứng dụng ấn định trong thư mục riêng. macOS và Windows tự động tải thành phần cần thiết; gói Linux có sẵn môi trường chạy CPU. Người dùng hiện tại được hỏi trước khi chuyển sang môi trường do Vyact quản lý; thay đổi phiên bản cũng cần người dùng đồng ý. Các chương trình đã cài trên hệ thống được giữ nguyên.

| Nền tảng | Yêu cầu của ứng dụng chính | Yêu cầu theo tính năng |
| --- | --- | --- |
| macOS (Apple Silicon) | Không có | **GGUF cục bộ**: Vyact tự động tải và quản lý môi trường chạy với phiên bản cố định.<br><br>**MLX cục bộ**: Vyact cài oMLX trong môi trường riêng; cần macOS 15 trở lên và Git.<br><br>**Elasticsearch**: Chế độ chạy trực tiếp không cần phụ thuộc bên ngoài; Docker Desktop là tùy chọn cho chế độ vùng chứa.<br><br>**Kokoro TTS**: Chỉ cần Homebrew khi phải cài `espeak-ng`. |
| Windows | Không có | **GGUF cục bộ**: Vyact tự động tải và quản lý môi trường chạy với phiên bản cố định.<br><br>**Elasticsearch**: Chế độ chạy trực tiếp không cần phụ thuộc bên ngoài; Docker Desktop là tùy chọn.<br><br>**Kokoro TTS**: Chỉ cần `winget` khi phải cài `espeak-ng`. |
| Linux (x64) | Môi trường máy tính x86-64 có glibc 2.35 trở lên; DEB cài thư viện giao diện đã khai báo qua APT. | **GGUF cục bộ**: Có sẵn môi trường chạy CPU.<br><br>**Elasticsearch**: Chế độ chạy trực tiếp không cần phụ thuộc bên ngoài; Docker là tùy chọn.<br><br>**Trình duyệt và Kokoro TTS**: Khi thiếu thư viện hoặc `espeak-ng`, cần trình quản lý gói được hỗ trợ (`apt-get`, `dnf`, `zypper`, `pacman`) và tác nhân xác thực PolicyKit trên máy. Vyact yêu cầu quyền qua `pkexec`; nếu không có, chỉ thử `sudo` không cần mật khẩu hoặc đã lưu xác thực. |
