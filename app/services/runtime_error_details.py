...
def runtime_startup_error(fallback: str, log_path: Path, *, since: int = 0) -> RuntimeStartupError:
    """Include a bounded log tail so callers can classify startup failures."""
    try:
        with log_path.open("rb") as log_file:
            log_file.seek(0, 2)
            log_file.seek(max(since, log_file.tell() - RUNTIME_ERROR_LOG_TAIL_BYTES))
